...
struct bst_iterator{
	struct bst_node* cur;
	struct bst_node* end; //used as a flag to signal that next has reached the last value 
	struct stack* stack; //stack to simulate call stack, stores bst nodes 
	
};

/*
 * This is a helper function for bst_size
 * Allows recursive calls, since bst* and bst_node* can not both be a parameter
 *
 * params: 
 * 		node - the node for which the size is being determined
 *
 * 	return: 
 * 		the size of the subtree, rooted at node
 */
int sub_size(struct bst_node* node){
	if(node){
		return 1 + sub_size(node->left) + sub_size(node->right); //1 is a counter, otherwise would always return 0
	}
	else {
		return 0;
	}
 ...
 * params:
 * 	node: the bst_node* for which the sub-height is computed
 *
 * returns: 
 * 	the height of the subtree rooted at the node 
 */
int sub_height(struct bst_node* node){
	if(node && (node->left || node->right) ){
		if(sub_height(node->left) > sub_height(node->right)){
			return 1 + sub_height(node->left); 
		}
		else {
			return 1 + sub_height(node->right);
		}
	}
	else{
		return 0;
 ...
 *
 * params:
 * 	sum: the path sum that is desired
 * 	node: the root node of the (sub)tree
 *
 * returns:
 * 	integer, 1 or 0 if path exists or does not
 */
int get_sum(int sum, struct bst_node* node){
	int sum_l = 0, sum_r = 0; //stores the sums down the left and right nodes

	//has left or right node
	int has_l = (node->left != NULL); 
	int has_r = (node->right != NULL);
	
	if(has_l || has_r){
		if(has_l && get_sum(sum - node->val, node->left) == sum - node->val){
			return sum; 
		}
		if(has_r && get_sum(sum - node->val, node->right) == sum - node->val){
			return sum;
		}
	}
	else if(node->val == sum){ //node is guaranteed to be a leaf by this point
		return node->val;
	}
	else {
		return sum - 1; //arbitrary value, can't use 0 or we get false positives 
	}
}
...
int bst_path_sum(int sum, struct bst* bst) {
	return (get_sum(sum, bst->root) == sum);
}
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
	...
	iter->cur = bst->root; 
	iter->end = malloc(sizeof(struct bst_node));
	return iter; 
}
...
/*
 * does an in-order tour around the tree, adding values to the stack
 *
 * params:
 * 	iter - the iterator for which the stack will be populated
 * 	node - the starting node for the tour
 */
void tour(struct bst_iterator* iter, struct bst_node* node){
	if(node){
		tour(iter, node->right);
...
int bst_iterator_has_next(struct bst_iterator* iter) {
	...
	//if this is the first use of the iterator 
	if(stack_isempty(iter->stack)){
		tour(iter, iter->cur); 	
	}
	else if(stack_top(iter->stack) == iter->end){
		return 0; 
	}
...
int bst_iterator_next(struct bst_iterator* iter) {
	...
	//if first use of iterator (should never be true if iterator used properly) 
	if(stack_isempty(iter->stack)){
		tour(iter, iter->cur); 	
	}
	else{
		int* val = stack_pop(iter->stack);
		if(stack_isempty(iter->stack)){
			stack_push(iter->stack, iter->end); //end is used as a flag, signals the last value in the order
		}
		return *val;
	}
}
